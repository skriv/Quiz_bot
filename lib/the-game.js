...
const NEW_QUESTION_DELAY = 1000; //15000
const MESSAGE_TYPING_DELAY = 1000;
const HINTS_DELAY = 13000; //13000
...
class Game {
  ...
  send(text) {
    return this.bot.sendChatAction(this.chat.id, 'typing')
      ...
      .then(() => this.bot.sendMessage(this.chat.id, text, { //ADD
        parse_mode: 'Markdown'
      }
    ));
  }
...
//add error messages
  onErrorAnswer(msg){
    return this.replyOn(msg, messages.errorAnswer(this.currentPlayer));
  }
